...
<section class="office-locater-one olc-layout <?php echo esc_attr($olcMapAttr['map_layout']); ?>" data-id="<?php echo esc_attr( $olc_section_id ); ?>" id="<?php echo esc_attr( $olc_section_id ); ?>">
    <div class="olc-container">
        ...
        <style> 
            <?php 
            $olc_style_5 = "#".$olc_section_id." .olc-google-map{  width:".$olcMapAttr['map_width']." !important; height:".$olcMapAttr['map_height']." !important; } #".$olc_section_id." .office-panel{ height:".$olcMapAttr['map_height']."; } #".$olc_section_id.".olc-layout { max-width: ".$map_container_size." !important; background:".$olcMapAttr['map_background_color']."; } @media only screen and (max-width: 767px){ #".$olc_section_id." .olc-google-map, #".$olc_section_id." .office-panel {height: 500px  !important;width: 100%  !important; }} @media only screen and (max-width: 575px){ #".$olc_section_id." .office-panel,#".$olc_section_id." .olc-google-map {height: 400px  !important;}}";

            $olc_style_5 = apply_filters( 'olc_map_inline_style_5', $olc_style_5, $olc_section_id, $olcMapAttr );
            echo esc_html( $olc_style_5 );
            ?>
        </style>
        <?php
        ...
        if( isset($ofcGeneralData['map_api_key']) && !empty( $ofcGeneralData['map_api_key'] ) ){
            ?>
            <div class="office-locater-two olc-col-2">
                <div class="olc-google-map"></div>
                <div class="office-panel-with-filter">
                    ...
                    <?php if( isset( $olcMapAttr['enable_store_office'] ) && !empty( $olcMapAttr['enable_store_office'] ) && $olcMapAttr['enable_store_office'] == 'yes' ){ ?>  
                        <div class="office-locater-one-left office-panel" id="office-panel"></div>
                    <?php }  ?>
                </div>
            </div>
            <?php
